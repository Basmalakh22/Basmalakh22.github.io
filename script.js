...
class MobileMenuManager
{
    ...
    handleOutsideClick(e)
    {
        ...
        const clickedInsideMenu = elements.mobileMenu.contains(e.target);
        const clickedMenuButton = elements.mobileMenuButton && elements.mobileMenuButton.contains(e.target);
...
class BackToTopManager
{
    ...
    updateProgress()
    {
        ...
        const completion = scrollableHeight === 0 ? 100 : Math.min(100, Math.round((window.pageYOffset / scrollableHeight) * 100));
        const progressAngle = `${Math.round((completion / 100) * 360)}deg`;

        elements.backToTopButton.style.setProperty('--scroll-progress-angle', progressAngle);
        elements.backToTopButton.setAttribute('aria-label', `Back to top. Page completion ${completion}%`);
    }
    ...
    triggerRocketLaunch()
    {
        ...
        elements.backToTopButton.classList.remove('is-launching');
        void elements.backToTopButton.offsetWidth;
        elements.backToTopButton.classList.add('is-launching');

        if (this.launchResetTimeout)
        {
            clearTimeout(this.launchResetTimeout);
        }

        this.launchResetTimeout = setTimeout(() =>
        {
            elements.backToTopButton.classList.remove('is-launching');
        }, 1100);
    }
...
class TiltEffectManager
{
    ...
    handleTilt(e)
    {
        ...
        // Keep card position fixed and only animate frame/shadow based on pointer position.
...
class CustomCursorManager
{
    ...
    createCursorElements()
    {
        ...
        // Create click burst layer
        this.cursorClickBurst = document.createElement('div');
        this.cursorClickBurst.className = 'cursor-click-burst';
        document.body.appendChild(this.cursorClickBurst);
    ...
    triggerClickBurst()
    {
        if (!this.cursorClickBurst) return;

        this.cursorClickBurst.style.left = `${this.mouseX}px`;
        this.cursorClickBurst.style.top = `${this.mouseY}px`;

        // Restart animation each click.
        this.cursorClickBurst.classList.remove('active');
        void this.cursorClickBurst.offsetWidth;
        this.cursorClickBurst.classList.add('active');
    }
    ...
    startAnimation()
    {
        const animateCursor = () =>
        {
            ...
            // Move with left/top so CSS transform animations (hover/click/rotation) stay intact
            const dotOffset = this.cursorDot.offsetWidth / 2;
            const outlineOffset = this.cursorDotOutline.offsetWidth / 2;

            this.cursorDot.style.left = `${this.dotX - dotOffset}px`;
            this.cursorDot.style.top = `${this.dotY - dotOffset}px`;

            this.cursorDotOutline.style.left = `${this.outlineX - outlineOffset}px`;
            this.cursorDotOutline.style.top = `${this.outlineY - outlineOffset}px`;
    ...
    destroy()
    {
        ...
        if (this.cursorClickBurst)
        {
            this.cursorClickBurst.remove();
        }
    }
}
